...
public class DefaultValueRegistry {
    ...
    public static func defaultValue<T>(for type: Array<T>.Type) -> [T] {
        return defaultValueOrNil(for: type) ?? []
    }

    public static func defaultValue<K, V>(for type: Dictionary<K, V>.Type) -> [K: V] {
        return defaultValueOrNil(for: type) ?? [:]
    }

    public static func defaultValue<T>(for type: Optional<T>.Type) -> T? {
